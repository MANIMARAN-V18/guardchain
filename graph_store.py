...
class GraphStore:
    ...
    def save_case_and_edges(self, root_wallet, chain, freeze_score, edges, case_id=None):
        """
        Saves all traced edges AND registers a Case node linking all involved wallets.
        This enables cross-investigation cluster detection.
        """
        timestamp = datetime.now(timezone.utc).isoformat()
        if not case_id:
            case_id = f"CASE-{chain[:3].upper()}-{root_wallet[:8]}-{int(datetime.now().timestamp())}"

        # 1. Update local cache
        cache = _read_local_cache()
        wallets_in_trace = list({e[0] for e in edges} | {e[1] for e in edges}) if edges else [root_wallet]
        exchanges_in_trace = [e[1] for e in edges if (e[4] if len(e) > 4 else False)]

        # Remove duplicate case if exists
        cache = [c for c in cache if c.get("case_id") != case_id]
        cache.append({
            "case_id": case_id,
        ...
        try:
            ...
            for edge in edges:
                ...
                self.save_edge(f_addr, t_addr, val, hop, is_ex, chain=chain)

            # Create Case node and link
            case_query = """
                ...
                with self.driver.session() as session:
                    ...
                    for r in result:
                        records.append({
                            ...
                            "root_2": r["root2"],
                            "chain_2": r["chain2"],
                            "shared_wallets": [w for w in r["shared_wallets"] if w],
                            "shared_exchanges": [e for e in r["shared_exchanges"] if e],
                            "overlap_count": len(r["shared_wallets"])
                        })
                    if records:
                        return {
